...
;===========================================================
;	TEMPORARY - hacked-in loader per DE: skips the normal low-RAM
;	bounce entirely (both _bigglesReadSect and its byte-serving API),
;	reading the SD sector trap direct and DMA-hopping straight from
;	the hypervisor's own trap buffer ($FFD6E00, same source as
;	lstBigglesDMA above) to any far destination the caller sets up
;	first. Not the real block-transfer API - that's still to be
;	designed.
;
;	Originally font-specific (destination hardcoded to $FF7E000) -
;	generalised once a second caller (chess piece graphics, going to
;	$010800) turned up. Destination is a 28-bit far address split
;	MEGA65/C65-style across the DMA job: bigglesFarDstMB is the top
;	byte (the $81 option's value - same field lstBigglesFarDMA's
;	source $80 option always sets to $FF), bigglesFarDstBank is the
;	next nybble (stored as a full byte, only the low nybble is used),
;	bigglesFarDst is the bottom 16 bits. Concatenating their hex
;	digits MB|bank|word gives the plain far address, e.g.
;	MB=$FF, bank=$07, word=$E000 -> $FF7E000.
;===========================================================

bigglesFarSectorsLeft:
		.byte	$00

lstBigglesFarDMA:
	;; MEGA65 Enhanced DMA options
        .byte $0A		;; Request format is F018A
        .byte $80,$FF		;; Source is $FFxxxxx (SD sector trap buffer)
bigglesFarDstMB:
        .byte $81,$00		;; Destination MB - caller sets the value byte
        .byte $00		;; No more options
        ;; F018A DMA list
        .byte $00		;; copy + last request in chain
        .word $0200		;; size of copy is 512 bytes
        .word $6E00		;; source $6E00...
        .byte $0D		;; ...of bank $D - same as lstBigglesDMA
bigglesFarDst:
        .word $0000		;; dest word offset - caller sets the start,
				;; advanced by $0200 each sector
bigglesFarDstBank:
        .byte $00		;; dest bank nybble - caller sets the start
        .word $0000		;; modulo (unused)


;-----------------------------------------------------------
bigglesLoadFarHack:
;	.A			IN	Number of 512-byte sectors to read
;	bigglesFarDstMB + 1	IN	Destination MB (top byte)
;	bigglesFarDstBank	IN	Destination bank (low nybble)
;	bigglesFarDst		IN	Destination starting word offset
;
;	.C		OUT	Set if error
;-----------------------------------------------------------
		STA	bigglesFarSectorsLeft

@sector:
		LDA	#$1A			;dos_readsector trap - fills
		STA	$D640			;the SD sector buffer at $FFD6E00
		CLV

		STX	sizBigglesBuf		;trap returns bytes read here
		STY	sizBigglesBuf + 1
